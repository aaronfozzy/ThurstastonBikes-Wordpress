...
- moving custom ahoy function to main functions file
...
- removed "Powered by Wordpress & custom" from footer, because let's face it: we all delete this anyway.
...
- removed default "custom" text in footer if you don't have a menu
...
- added custom body class depending on what browser
...
- added post formats to custom.php core
...
- fixed error in custom.php for menu fallback **thanks Mark**
...
- moved the custom script call and pngfix to the footer
	from the custom file so it's easier to see what's being
	called.
